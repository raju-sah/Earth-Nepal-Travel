        ...
        <div class="card">
            <div class="card-body">
                ...
                <x-form.wrapper action="{{route('admin.users.update', $user->id)}}" method="POST" enctype="multipart/form-data">
                    ...
                    <x-form.row>
                        <x-form.input type="text" :req="true" col="6" label="Name" id="name" name="name" value="{{$user->name}}"/>
                        <x-form.input type="text" :req="true" col="6" label="Email" id="email" name="email" value="{{$user->email}}"/>
                    </x-form.row>
                            ...
                            <div id="user_permissions_checkbox_list" class="d-flex flex-wrap">
                                @foreach ($rolePermissions as $permission)
                                    <div class="form-check mx-1 my-2">
                                        <input class="form-check-input" style="margin-right:5px" type="checkbox" name="permissions[]" id="{{$permission->id}}"
                                               value="{{$permission->id}}" {{ in_array($permission->id, $userPermissions, true) ? 'checked' : '' }}>
                    ...
                    <x-form.checkbox label="Status" id="status" name="status" value="1" class="form-check-input" isEditMode="yes" :isChecked="$user->status ? 'checked' : ''"/>
                ...
                $.ajax({
                    ...
                    $.each(data, function (index, element) {
                        $(permissions_checkbox_list).append(
                            '<div class="form-check mx-1 my-2">' +
                            '<input class="form-check-input" style="margin-right:5px" type="checkbox" checked name="permissions[]" id="' + element.title + '" value="' + element.id + '">' +
